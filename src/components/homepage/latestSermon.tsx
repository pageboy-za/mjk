...
export default function YouTubePlaylist({ }: YouTubePlaylistProps) {
  ...
  return error ? (
    ...
    <>
      <Container>
        <div className="py-4">
          Watch our latest sermon here, if you missed a different one or want to
          watch one again have a look here on this{" "}
          <Link href="/youtube" className="font-semibold hover:underline">
            page
          </Link>
          .
        </div>
        ...
        <div id="video">
          {videoId && (
            <VideoComponent
              vidSrc={"https://www.youtube.com/watch?v=" + videoId}
              controls={true}
            />
          )}
        </div>
